...
def shell(title, content, repository_url=None):
...
<meta name="robots" content="noindex,nofollow"><meta name="description" content="An open library of reusable workflows for AI agents. Find a job, inspect the steps, and contribute improvements.">
...
<body><a class="skip" href="#main">Skip to content</a>''' + notice + '''<div class="shell"><header><a class="brand" href="/">agentworkflows<span>.wiki</span></a><nav aria-label="Main"><a href="/#workflows">Workflows</a><a href="/contribute/">Contribute</a>''' + repo + '''</nav></header><main id="main">''' + content + '''</main><footer><span>Open workflows. Shared knowledge.</span><nav aria-label="Resources"><a href="/catalog.json">Catalog JSON</a><a href="/llms.txt">For agents</a><a href="/LICENSE.txt">MIT license</a></nav></footer></div></body></html>'''


def homepage(records, repository_url=None):
    rows = []
    for item in records:
...
def workflow_page(item, repository_url=None):
    ...
    note = ('A local example check is recorded. It does not establish general compatibility or safety.'
            if checked else 'This guide has not been demonstrated end to end. The examples are illustrations, not test results.')
    ...
    content += '<div class="detail-layout"><article class="prose">' + markdown(item['instructions']) + '<section id="examples"><h2>See the example</h2><p class="muted">Read the input beside the expected shape of the result. Check the labels for what was actually executed.</p><details><summary>Example input</summary>' + markdown(item['example_input']) + '<a href="' + base + 'examples/input.md">Read input Markdown ↗</a></details><details><summary>Example output</summary>' + markdown(item['example_output']) + '<a href="' + base + 'examples/output.md">Read output Markdown ↗</a></details></section></article>'
    ...
    return shell(item['title'], content, repository_url)
