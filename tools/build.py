...
ASSET_FILES = ('site.css', 'site.js', 'icon.svg', 'share-card.png')
...
def _render(root, dist, records, repository_url):
    ...
    walkthrough = root / 'workflows/release-notes-digest/examples/real-run.md'
    demo_url = '/examples/release-notes-digest/' if walkthrough.is_file() else None
    put('index.html', homepage(records, repository_url, demo_url=demo_url))
    if demo_url:
        body = markdown(walkthrough.read_text(encoding='utf-8'))
        body = body.replace('<h2>', '<h1>', 1).replace('</h2>', '</h1>', 1)
        content = '<a class="back" href="/workflows/release-notes-digest/">← Use this workflow</a><article class="prose standalone">' + body + '</article>'
        put('examples/release-notes-digest/index.html', shell('A real release digest', content, repository_url,
            description='A bounded agent run on official Python release notes, with source gaps and human review kept explicit.', canonical_path=demo_url))
    catalog = []
    for record in records:
        sid = record['id']
        source = root / 'workflows' / sid
        base = 'workflows/' + sid + '/'
        put(base + 'index.html', workflow_page(record, repository_url, demo_url=demo_url if sid == 'release-notes-digest' else None))
